...
- **PDF 스마트 변환**: PDF 파일을 업로드하면 고화질 이미지와 함께 텍스트 레이어 및 하이퍼링크가 자동으로 추출됩니다.
- **원본 링크 유지**: PDF 내의 하이퍼링크가 HTML `<a>` 태그로 완벽하게 변환되어 클릭 가능합니다.
- **클라이언트 사이드 처리**: 빠르고 안전한 브라우저 기반 PDF 처리로 서버 부하를 최소화합니다.
